                ...
                size_t split)
{
  ...
  for (size_t i = 0; i < total_length; i++) {
    ...
    seq[pos++] = alphabet[(state >> 16) & 0x3u];
  }
         ...
         unsigned int  repeats)
{
  ...
  printf("%s,%s,%zu,%u,%.6f,%llu,%llu,%llu,%llu,%llu,%llu,%llu,%llu\n",
         dataset,
         variant,
         strlen(sequence) - 1,
         repeats,
     ...
     char *argv[])
{
  ...
  printf("dataset,variant,total_length,repeats,dG,wall_ns,fill_arrays_ns,decompose_pair_ns,internal_ns,multibranch_ns,exterior_ns,bpp_ns,allocations\n");
