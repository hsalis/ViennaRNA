         ...
         unsigned int  repeats)
{
  ...
  for (r = 0; r < repeats; r++) {
    ...
    vrna_exp_params_rescale(fc2, &mfe);

    t0 = now_ns();
    sol = vrna_pf_TwoD(fc2, -1, -1);
    wall_sum += now_ns() - t0;

    if (sol) {
      for (i = 0; sol[i].k != INF; i++)
        q_sum += (double)sol[i].q;
      classes_sum += i;
      free(sol);
    }
     ...
     char *argv[])
{
  ...
  printf("length,repeats,wall_ns,mean_q_sum,mean_classes\n");
