...
PRIVATE int
E_mb_loop_fast_common(vrna_fold_compound_t         *fc,
                      unsigned int                 i,
                      unsigned int                 j,
                      struct vrna_mx_mfe_aux_ml_s  *helpers);
...
use_multibranch_single_fast_path(vrna_fold_compound_t *fc);


PRIVATE INLINE unsigned char
use_multibranch_soft_constraints(vrna_fold_compound_t *fc);


PRIVATE INLINE unsigned int
ptype_fast_mb(vrna_md_t    *md,
              short        *S2,
              unsigned int i,
              unsigned int j);
...
PRIVATE INLINE unsigned char
hc_mb_linear_fast(vrna_hc_t      *hc,
                  unsigned int   n,
                  unsigned int   i,
                  unsigned int   j,
                  unsigned int   k,
                               ...
                               struct vrna_mx_mfe_aux_ml_s  *helpers)
{
  if ((fc) &&
    ...
    ret = E_mb_loop_fast(fc,
                         i,
                         j,
                         helpers);

    if (vrna_mfe_profile_enabled())
      vrna_mfe_profile_add_multibranch(vrna_mfe_profile_now() - t0);
                             ...
                             struct vrna_mx_mfe_aux_ml_s  *helpers)
{
  if ((fc) &&
    ...
    ret = mfe_multibranch_m2_fast(fc, i, j, helpers);
                        ...
                        unsigned int          j)
{
  ...
  if ((fc) &&
    ...
    unsigned char         use_sc = use_multibranch_soft_constraints(fc);
    ...
    e = extend_fm_3p(i, j, fc->matrices->fM1, fc, &sc_wrapper);
...
get_aux_arrays(unsigned int length)
{
  ...
  aux->Fmi_tmp = (int *)vrna_alloc(sizeof(int) * (length + 2)); /* reusable scratch for constrained decompositions */
...
ptype_fast_mb(vrna_md_t    *md,
              ...
              unsigned int j)
{
  unsigned int tt = (unsigned int)md->pair[S2[j]][S2[i]];
                  ...
                  unsigned char  d)
{
  ...
  switch (d) {
    ...
    case VRNA_DECOMP_ML_COAXIAL:
      return (hc->mx[n * k + l] & VRNA_CONSTRAINT_CONTEXT_MB_LOOP_ENC) ? 1 : 0;
  }
...
PRIVATE INLINE int
ml_pair_d0_fast_common(vrna_fold_compound_t *fc,
                       unsigned int         i,
                       unsigned int         j,
                       int                  *dmli1)
{
  ...
  n = fc->length;
  if (!hc_mb_linear_fast(fc->hc, n, i, j, i + 1, j - 1, VRNA_DECOMP_PAIR_ML))
    return INF;
  ...
  tt  = ptype_fast_mb(md, S2, i, j);

  if (md->noGUclosure && ((tt == 3) || (tt == 4)))
    return INF;

  return e + vrna_E_multibranch_stem(tt, -1, -1, P) + P->MLclosing;
}


PRIVATE INLINE int
ml_pair_d2_fast_common(vrna_fold_compound_t *fc,
                       unsigned int         i,
                       unsigned int         j,
                       int                  *dmli1)
{
  ...
  return e + vrna_E_multibranch_stem(tt, S[j - 1], S[i + 1], P) + P->MLclosing;
}


PRIVATE INLINE int
ml_pair5_fast_common(vrna_fold_compound_t *fc,
                     unsigned int         i,
                     unsigned int         j,
                     int                  *dmli2)
{
  ...
  n = fc->length;
  if (!hc_mb_linear_fast(fc->hc, n, i, j, i + 2, j - 1, VRNA_DECOMP_PAIR_ML))
    return INF;
  ...
  return e + vrna_E_multibranch_stem(tt, -1, S[i + 1], P) + P->MLclosing + P->MLbase;
}


PRIVATE INLINE int
ml_pair3_fast_common(vrna_fold_compound_t *fc,
                     unsigned int         i,
                     unsigned int         j,
                     int                  *dmli1)
{
  ...
  n = fc->length;
  if (!hc_mb_linear_fast(fc->hc, n, i, j, i + 1, j - 2, VRNA_DECOMP_PAIR_ML))
    return INF;
  ...
  return e + vrna_E_multibranch_stem(tt, S[j - 1], -1, P) + P->MLclosing + P->MLbase;
}


PRIVATE INLINE int
ml_pair53_fast_common(vrna_fold_compound_t *fc,
                      unsigned int         i,
                      unsigned int         j,
                      int                  *dmli2)
{
  ...
  n = fc->length;
  if (!hc_mb_linear_fast(fc->hc, n, i, j, i + 2, j - 2, VRNA_DECOMP_PAIR_ML))
    return INF;
  ...
  return e + vrna_E_multibranch_stem(tt, S[j - 1], S[i + 1], P) + P->MLclosing + 2 * P->MLbase;
}
           ...
           struct sc_mb_dat     *sc_wrapper)
{
  ...
  e = ml_pair_d0(fc, i, j, dmli1, sc_wrapper);

  /* new closing pair (i,j) with mb part [i+2,j-1] */
  en  = ml_pair5(fc, i, j, dmli2, sc_wrapper);
  e   = MIN2(e, en);

  /* new closing pair (i,j) with mb part [i+1, j-2] */
  en  = ml_pair3(fc, i, j, dmli1, sc_wrapper);
  e   = MIN2(e, en);

  /* new closing pair (i,j) with mb part [i+2.j-2] */
  en  = ml_pair53(fc, i, j, dmli1, dmli2, sc_wrapper);
  e   = MIN2(e, en);
         ...
         struct sc_mb_dat     *sc_wrapper)
{
  ...
  if (hc->eval_mb(i, j, i + 2, j - 1, VRNA_DECOMP_PAIR_ML, hc)) {
    e = dmli2[j - 1];
         ...
         struct sc_mb_dat     *sc_wrapper)
{
  ...
  if (hc->eval_mb(i, j, i + 1, j - 2, VRNA_DECOMP_PAIR_ML, hc)) {
    e = dmli1[j - 2];
          ...
          struct sc_mb_dat      *sc_wrapper)
{
  ...
  if (hc->eval_mb(i, j, i + 2, j - 2, VRNA_DECOMP_PAIR_ML, hc)) {
    e = dmli2[j - 2];
                      ...
                      struct vrna_mx_mfe_aux_ml_s  *helpers)
{
  ...
  switch (dangle_model) {
    case 0:
      return ml_pair_d0_fast_common(fc, i, j, dmli1);

    case 2:
      return ml_pair_d2_fast_common(fc, i, j, dmli1);

    default:
      decomp  = ml_pair_d0_fast_common(fc, i, j, dmli1);
      en      = ml_pair5_fast_common(fc, i, j, dmli2);
      decomp  = MIN2(decomp, en);
      en      = ml_pair3_fast_common(fc, i, j, dmli1);
      decomp  = MIN2(decomp, en);
      en      = ml_pair53_fast_common(fc, i, j, dmli2);
      decomp  = MIN2(decomp, en);
      return decomp;
  }
}
               ...
               struct vrna_mx_mfe_aux_ml_s  *helpers)
{
  ...
  dmli1         = helpers->DMLi1;
  dmli2         = helpers->DMLi2;
  use_sc        = use_multibranch_soft_constraints(fc);

  /* init values */
  decomp = INF;

  if ((!use_sc) && use_multibranch_single_fast_path(fc))
    return E_mb_loop_fast_common(fc, i, j, helpers);
  ...
  switch (dangle_model) {
    /* no dangles */
    case 0:
      decomp = ml_pair_d0(fc, i, j, dmli1, &sc_wrapper);
      break;

    /* double dangles */
    case 2:
      decomp = ml_pair_d2(fc, i, j, dmli1, &sc_wrapper);
      break;

    /* normal dangles, aka dangles = 1 || 3 */
    default:
      decomp = ml_pair_d1(fc, i, j, dmli1, dmli2, &sc_wrapper);
      break;
  }
                ...
                unsigned int          j)
{
  ...
  if (evaluate(i, j, i + 1, j - 1, VRNA_DECOMP_PAIR_ML, hc)) {
    decomp = INF;
    if (sliding_window) {
      for (k = i + 2; k < j - 2; k++) {
        if (evaluate(i, j, i + 1, k, VRNA_DECOMP_ML_COAXIAL, hc)) {
        ...
        if (evaluate(i, j, k + 1, j - 1, VRNA_DECOMP_ML_COAXIAL, hc)) {
             ...
             struct sc_mb_dat     *sc_wrapper)
{
  ...
  if (dangle_model % 2) {
    if ((i + 1 < j) &&
        (evaluate(i, j, i + 1, j, VRNA_DECOMP_ML_STEM, hc))) {
    ...
    if ((i + 1 < j) &&
        (evaluate(i, j, i, j - 1, VRNA_DECOMP_ML_STEM, hc))) {
    ...
    if ((i + 2 < j) &&
        (evaluate(i, j, i + 1, j - 1, VRNA_DECOMP_ML_STEM, hc))) {
  ...
  if (with_ud) {
    for (cnt = 0; cnt < (unsigned int)domains_up->uniq_motif_count; cnt++) {
      ...
      k = j - u + 1;
      if ((k > i) && (evaluate(i, j, i, k - 1, VRNA_DECOMP_ML_ML, hc))) {
                ...
                struct vrna_mx_mfe_aux_ml_s *helpers)
{
   ...
   */
  e = extend_fm_3p(i, j, NULL, fc, &sc_wrapper);
   ...
   */
  if ((i + 1 < j) &&
      (evaluate(i, j, i + 1, j, VRNA_DECOMP_ML_ML, hc))) {
  ...
  if (with_ud) {
    for (cnt = 0; cnt < (unsigned int)domains_up->uniq_motif_count; cnt++) {
      u = domains_up->uniq_motif_size[cnt];
      k = i + u - 1;
      if ((k < j) && (evaluate(i, j, k + 1, j, VRNA_DECOMP_ML_ML, hc))) {
  ...
  if (dangle_model == 3) {
    ...
    if (sliding_window) {
      /* additional ML decomposition as two coaxially stacked helices */
      for (decomp = INF, k = i + 1; k <= j - 2; k++) {
        if (evaluate(i, k, k + 1, j, VRNA_DECOMP_ML_COAXIAL_ENC, hc)) {
