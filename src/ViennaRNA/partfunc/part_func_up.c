 ...
 */
PRIVATE pu_out *
get_u_vals(pu_contrib *p_c,
           int        **unpaired_values,
           char       *select_contrib);


PRIVATE int
plot_free_pu_out(pu_out   *res,
                 interact *pint,
                 char     *ofile,
                 char     *head);
...
PRIVATE void
scale_stru_pf_params(unsigned int length);


PRIVATE void
init_pf_two(int length);


PRIVATE void
scale_int(const char  *s,
          const char  *sl,
          double      *sc_int);


PRIVATE void
encode_seq(const char *s1,
           const char *s2);


PRIVATE constrain *
get_ptypes_up(char        *S,
              const char  *structure);


PRIVATE void
get_up_arrays(unsigned int length);
...
PRIVATE void
set_encoded_seq(const char  *sequence,
                short       **S,
                short       **S1);


PRIVATE void
get_interact_arrays(unsigned int  n1,
                    unsigned int  n2,
                    pu_contrib    *p_c,
                    pu_contrib    *p_c2,
                    int           w,
 ...
 */
PUBLIC pu_contrib *
get_pu_contrib_struct(unsigned int  n,
                      unsigned int  w)
{
  unsigned int  i;
            ...
            int         incr5)
{
  ...
  if (fold_constrained && cstruc != NULL) {
    ...
    if (pos) {
      ...
      pos         = strchr(i_long, '|');
      if (pos)
        ck = (int)(pos - i_long) + 1;    /* k */

      pos = strrchr(i_long, '|');
      if (pos)
        ci = (int)(pos - i_long) + 1;    /* i */

      pos = strrchr(i_short, '|');
      if (pos)
        cl = (int)(pos - i_short) + 1;    /* l */
  ...
  for (i = 1; i <= n1; i++) {
    ...
    for (j = n2; j > 0; j--) {
      ...
      if (fold_constrained && pos && cl && j < cl - w + 1)
        break;
      ...
      qint_4[i][j][0][0] *= vrna_exp_E_exterior_stem(type,
                                          (i > 1) ? S1[i - 1] : -1,
                                          (j < n2) ? SS2[j + 1] : -1,
                                          Pf);

      rev_d = vrna_exp_E_exterior_stem(rtype[type], (j > 1) ? SS2[j - 1] : -1, (i < n1) ? S1[i + 1] : -1, Pf);
      ...
      if (p_c2 == NULL) {
        ...
        /* consider structures of both seqs. */
        qint_ik[i][i] += qint_4[i][j][0][0] * rev_d * p_c_S[add_i5][add_i3] * p_c2_S[j][0] *
                         scale[((int)w / 2)];
        Z += qint_4[i][j][0][0] * rev_d * p_c_S[add_i5][add_i3] * p_c2_S[j][0] *
             scale[((int)w / 2)];
      }
      ...
      for (k = i - 1; k > end_k && k > 0; k--) {
        ...
        for (l = j + 1; l < end_l && l <= n2; l++) {
          ...
          if (fold_constrained && pos && (k <= ck || i >= ci) && !type2)
            continue;

          if (fold_constrained && pos && ((cstruc[k - 1] == '|') || (cstruc[n1 + l - 1] == '|')) &&
              !type2)
            break;
          ...
          if (fold_constrained && pos && ci && i < ci)
            continue;

          /* collect interactions ending after 3' most '|' constrain*/
          if (fold_constrained && pos && ck && k > ck)
            continue;

          /* '|' constrain in short sequence */
          /* collect interactions starting before 5' most '|' constrain */
          if (fold_constrained && pos && cj && j > cj)
            continue;

          /* collect interactions ending after 3' most '|' constrain*/
          if (fold_constrained && pos && cl && l < cl)
            continue;
    ...
    if (i > w) {
      int bla;
      bla = i - w;
      if (fold_constrained && pos && ci && i - w < ci - w + 1)
        continue;
           ...
           char       *select_contrib)
{
  ...
  for (count = k = 1; k <= unpaired_values[0][0]; k++) {
    ...
    do {
      ...
      if (S)
        sprintf(u_results->header[offset + off_S], "u%dS", w);

      if (E)
        sprintf(u_results->header[offset + off_E], "u%dE", w);

      if (H)
        sprintf(u_results->header[offset + off_H], "u%dH", w);

      if (I)
        sprintf(u_results->header[offset + off_I], "u%dI", w);

      if (M)
        sprintf(u_results->header[offset + off_M], "u%dM", w);

      if (p_c != NULL) {
        for (i = 1; i <= len; i++) {
          /* for each position */
          /* w goes form j to i (intervall end at i) */
          for (j = i; j < MIN2((i + w), len + 1); j++) {
            /* for each -u value < w
            ...
            if ((j - i + 1) == w && i + w - 1 <= len) {
              ...
              if (S)
                u_results->u_values[offset + off_S][i + w - 1] += blubb;

              if (E)
                u_results->u_values[offset + off_E][i + w - 1] += p_c->E[i][j - i];

              if (H)
                u_results->u_values[offset + off_H][i + w - 1] += p_c->H[i][j - i];

              if (I)
                u_results->u_values[offset + off_I][i + w - 1] += p_c->I[i][j - i];

              if (M)
                u_results->u_values[offset + off_M][i + w - 1] += p_c->M[i][j - i];
            }

            if (i < w && (j - i + 1) != w && i + w - 1 > len && i + w - 1 < len + 3) {
              if (S)
                u_results->u_values[offset + off_S][i + w - 1] = -1;

              if (E)
                u_results->u_values[offset + off_E][i + w - 1] = -1;

              if (H)
                u_results->u_values[offset + off_H][i + w - 1] = -1;

              if (I)
                u_results->u_values[offset + off_I][i + w - 1] = -1;

              if (M)
                u_results->u_values[offset + off_M][i + w - 1] = -1;
            }
          }
        }
      } else {
                 ...
                 char     *head)
{
  ...
  for (i = 0; i <= len; i++) {
    for (s = 0; s <= size + 1; s++) {
      /* that is for different contribution */
      ...
      if (i != 0) {
        if (s > 0 && s <= size) {
          if (res->u_values[s][i] > 0.0) {
            ...
            /* no p_u value was defined print nan*/
  ...
  if (fold_constrained && (structure != NULL)) {
    ...
    for (hx = 0, j = 1; j <= n; j++) {
      switch (structure[j - 1]) {
        case 'x': /* can't pair */
          for (l = 1; l < j - CO_TURN; l++)
            con->ptype[con->indx[l] - j] = 0;
          for (l = j + CO_TURN + 1; l <= n; l++)
            con->ptype[con->indx[j] - l] = 0;
          break;
        case '(':
          stack[hx++] = j;
        /* fallthrough */
